...
(() => {
  ...
  ROOMS.lobby = {
    ...
    hint() {
      ...
      if (G.money < 50) return "The penthouse doorman charges a $50 cover. The casino floor next door would love to 'help'.";
  ...
  ROOMS.casino = {
    ...
    hint() {
      if (G.money < 50) return "You need a bankroll ($50+ for the penthouse cover). Slots are fast; blackjack pays better if you keep your head. If you go fully broke, flag down the waitress.";
  ...
  ROOMS.penthouse = {
    ...
    hint() {
      if (!G.flags.minty) return "Before you talk to anyone up here — ESPECIALLY her — deploy the breath spray. Trust the hint. USE SPRAY.";
      const t3 = G.got.eva_talk3;
      if (!t3) return "TALK to Eva. Be honest. Be funny. Every sleazy line you've practiced tonight? Bury it.";
      if (!G.got.eva_apple) return "Eva, 4 AM, health nut, surrounded by champagne and canapés. Somewhere in your pocket is the one thing she actually wants. (It's the APPLE.)";
      if (!G.got.eva_wine) return "You've still got that wine. Two glasses, one skyline. GIVE WINE TO EVA.";
